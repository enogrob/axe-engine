...
class TFniBsc
  ...
  def initialize(a_node='BSCSPS2', a_io='APG', a_rev='R9.1', a_rev_next='R10MD')
    @node = a_node.upcase
    @io = a_io.upcase
    @rev = a_rev.upcase
...
#   end
        ...
        while line = srhtool_ini.gets
          if    line.include?('MainView.CreateA_SWR_Settings.LASIP_File=')
            ...
            if @apz[:apz_version].include?('3')
              srhtool_tmp.puts 'MainView.CompareTwoSWRsSettings.Reference_SWR=' + $SWR_R10_ACA05_21230_FILE
            else
              srhtool_tmp.puts 'MainView.CompareTwoSWRsSettings.Reference_SWR=' + $SWR_R10_ACA05_21225_FILE
            end
  ...
  def check_fceps
    ...
    if @apz[:apz_version].include?('3')
      rps_R11_ref = get_rps_swr($SWR_R11_ACA03_21230_FILE)
    else
      rps_R11_ref = get_rps_swr($SWR_R11_ACA03_21220_FILE)
    end
